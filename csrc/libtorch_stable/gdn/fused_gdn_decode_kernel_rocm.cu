...
load_state4<bf16_t>(const bf16_t* state) {
  return make_float4(__bfloat162float(state[0]), __bfloat162float(state[1]),
                     __bfloat162float(state[2]), __bfloat162float(state[3]));
}
...
template <>
__device__ __forceinline__ void store_state4<bf16_t>(
    bf16_t* state, float4 value) {
  state[0] = __float2bfloat16(value.x);
  state[1] = __float2bfloat16(value.y);
  state[2] = __float2bfloat16(value.z);
  state[3] = __float2bfloat16(value.w);
}
...
__global__ __launch_bounds__(kThreads, 1) void gdn_decode_post_conv_mtp_kernel(
...
#pragma unroll
    ...
    const float q_scale = __shfl(lane == 0 ? rsqrtf(qk_sums.x + 1.0e-6f) * scale : 0.0f, 0, 32);
    const float k_scale = __shfl(lane == 0 ? rsqrtf(qk_sums.y + 1.0e-6f) : 0.0f, 0, 32);
#pragma unroll
    for (int i = 0; i < 4; ++i) {
      const int dim = lane + i * 32;
...
#pragma unroll
    for (int i = 0; i < 4; ++i) {
      ...
      const float weight =
          norm_weight_is_bf16
              ? __bfloat162float(
                    static_cast<const bf16_t*>(norm_weight)[value])
...
void fused_gdn_decode_post_conv_mtp(
  ...
  STD_TORCH_CHECK(
      ...
      "dt_bias must be a GPU float32, bfloat16, or float16 tensor");
  STD_TORCH_CHECK(
  ...
  STD_TORCH_CHECK(
      ...
      "state must be a GPU float32 or bfloat16 tensor");
  ...
  STD_TORCH_CHECK(norm_weight.is_cuda() &&
                      (norm_weight.scalar_type() == ScalarType::Float ||
                       norm_weight.scalar_type() == ScalarType::BFloat16),
                  "norm_weight must be a GPU float32 or bfloat16 tensor");
